...
取得 WLAN 物件
如果目前已連線，則呼叫 wlan.disconnect() 並關閉 WLAN 介面
如果未連線，則印出提示訊息
is_connected() 檢查連線狀態
程式邏輯：
...
檢查是否已連線
如果已連線，回傳 IP 位址（wlan.ifconfig()[0]）
如果未連線，回傳 None
test_internet(host, port, timeout) 測試外部網路
程式邏輯：

使用 socket.getaddrinfo() 解析目標主機
建立 socket 連線並設定超時時間
嘗試連線到目標（預設為 Google DNS 8.8.8.8:53）
...
📚 參考資源
MicroPython network 模組文件
Raspberry Pi Pico W 官方文件
